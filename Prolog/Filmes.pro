/* Part 1: Write queries to answer the following questions.
    ...
    e. Find an actor who has appeared in more than one movie.
    	contar_aparicoes_ator(Ator, Total) :-
            findall(Filme, ator(Filme, Ator, _), Filmes),
            length(Filmes, Total).
    	ator(Filme, Ator, _), contar_aparicoes_ator(Ator, N), N > 1
    
    f. Find a director of a movie in which Scarlett Johansson appeared.
    	diretor(Filme, Diretor), atriz(Filme, scarlett_johansson, _)
    
    g. Find an actor who has also directed a movie.
    	diretor(Filme, Diretor), ator(_, Diretor, _)
    
    h. Find an actor or actress who has also directed a movie.
    	diretor(Filme, Diretor), (ator(_, Diretor, _) ; atriz(_, Diretor, _))
...
/* Part 2: Add rules to the database to do the following,
    ...
    c. same_year(M1, M2) <- the movies are released in the same year.
    	same_year(M1, M2):- filme(M1, A1), filme(M2, A2), M1 \= M2, A1 = A2.
    
    d. co_star(A1, A2) <- the actor/actress are in the same movie.
		ator_atriz(M, A, R):- ator(M, A, R) ; atriz(M, A, R).
		co_star(A1, A2):- ator_atriz(M, A1, _), ator_atriz(M, A2, _), A1 \= A2.
*/

/* Parte 3: Pesquise e proponha mais duas regras ou consultas sobre a base de filmes
   que considere interessante. Escreva o enunciado das regras propostas.
   a. Encontrar todos os filmes dirigidos por um diretor específico
       filmes_do_diretor(Diretor, Lista) :-
            findall(Filme, diretor(Filme, Diretor), Lista).  

   b. Encontrar todos os filmes em que uma pessoa participou como ator ou atriz
       filmes_com_participacao(Pessoa, Lista) :-
            findall(Filme, (ator(Filme, Pessoa, _) ; atriz(Filme, Pessoa, _)), Filmes),
            sort(Filmes, Lista).
*/

/* DATABASE
    filme(M, Y) <- filme M estreiou no ano Y
    diretor(M, D) <- filme M foi dirigido pelo diretor D
    ator(M, A, R) <- ator A interpretou R no filme M
    atriz(M, A, R) <- atriz A interpretou R no filme M
